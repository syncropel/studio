// /home/dpwanjala/repositories/syncropel/studio/src/widgets/GlobalView/index.tsx
"use client";

import React, { useState, useEffect, useCallback } from "react";
import { Box, Text, Loader, Center, Title, Button } from "@mantine/core";
...
/**
 * GlobalView is a focused component responsible for displaying and managing
...
export default function GlobalView() {
  ...
  const { currentPage, projectsTreeData, isWorkspaceLoading } =
    useSessionStore();
  const { cxHomeTreeData } = useSessionStore();
  ...
  // Fetch data for this view when it's first mounted
  useEffect(() => {
    if (readyState === ReadyState.OPEN && cxHomeTreeData.length === 0) {
      sendJsonMessage({
        type: "WORKSPACE.BROWSE",
        command_id: `browse-cxhome-${nanoid()}`,
        payload: { path: "cx_home://" },
      });
    }
  }, [readyState, cxHomeTreeData, sendJsonMessage]);
  ...
  const treeNodeIcon = (props: TreeNodeProps) => {
    ...
    if (nodeData.key.includes("secrets"))
      return <IconKey size={16} className="text-red-500" />;
    if (nodeData.isLeaf)
    ...
    <Box p="xs">
      <div className="minimalist-tree">
        ...
        <CollapsibleSection
          ...
          noPadding
        >
          {renderTree(cxHomeTreeData, "Could not load .cx directory.")}
        </CollapsibleSection>
